...
class SpectralConv1d(nn.Module):
    def __init__(self, in_channels, out_channels, init_threshold, signal_shape):
        ...
        self.weights = nn.Parameter(self.scale * torch.rand(in_channels, out_channels, signal_shape, dtype=torch.float))
    ...
    def compl_mul1d(self, input, weights):
        ...
        #print(input.shape)  # Should be (batch, in_channel, x)
        #print(weights.shape)  # Should be (in_channel, out_channel, x)
...
class ResNet_EEG(nn.Module):
    def __init__(self, ResBlock, SpectralConv1d, n_channal, init_threshold, num_classes):
        super(ResNet_EEG, self).__init__()

        self.init_threhsold = init_threshold
        self.channel = n_channal
        self.inchannel = 128
        self.conv1 = nn.Sequential(
            nn.Conv1d(n_channal, 128, kernel_size=7, stride=2, padding=1, bias=False),
            nn.BatchNorm1d(128),
            nn.ReLU()
        )
        ...
        self.fft64 = SpectralConv1d(128, 128, init_threshold = 0.2, signal_shape = 126) #498 126
        self.fft128 = SpectralConv1d(256, 256, init_threshold = 0.2, signal_shape = 63) #63
        self.fft256 = SpectralConv1d(512, 512, init_threshold = 0.2, signal_shape = 32) #32
        self.fft512 = SpectralConv1d(1024, 1024, init_threshold= 0.2, signal_shape = 16)  #16
