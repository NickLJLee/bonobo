...
# add transformations
transform_train = transforms.Compose([cut_and_jitter(windowsize=config.WINDOWSIZE,max_offset=0.1,Fq=config.FQ), channel_flip(p=0.5)])
...
trainer = pl.Trainer(devices=1, accelerator="gpu", min_epochs=50,max_epochs=150,logger=wandb_logger,callbacks=callbacks,fast_dev_run=False)
# train the model
