from torch.utils.data import DataLoader
import numpy as np
import pandas as pd
import os
import pickle
from torchvision import transforms
import pytorch_lightning as pl
import torch
from tqdm import tqdm

# load own code
import sys
sys.path.append('../')
from sleeplib.Resnet_15.model import FineTuning, SCDNN
from sleeplib.datasets import BonoboDataset, ContinousToSnippetDataset
# this holds all the configuration parameters
from sleeplib.config import Config
import pickle

from pytorch_lightning.callbacks import ModelCheckpoint
from torch.utils.data import DataLoader
from torchvision import transforms

from sleeplib.datasets import BonoboDataset , ContinousToSnippetDataset
from sleeplib.montages import CDAC_bipolar_montage,CDAC_common_average_montage,CDAC_combine_montage,con_combine_montage
from sleeplib.transforms import cut_and_jitter, channel_flip,extremes_remover
model_path = '/data/0shared/lijun/code/eeg/Spike_37chan/Models/YOUR_MODEL_NAME'
# load config and show all default parameters
config = Config()
# the config can be changed by setting config.PARAMETER = VALUE

# save config to model_path using pickle

path_model = '/data/0shared/lijun/code/eeg/Spike_37chan/Models/YOUR_MODEL_NAME'

# load dataset
df = pd.read_csv(config.PATH_LUT_BONOBO,sep=';')
# fraction filter
frac_filter = (df['fraction_of_yes'] > 6/8) | (df['fraction_of_yes'] < 2/8)
mode_filter = df['Mode'] == 'Test'
extreme_quality_filter = df['total_votes_received'] >= 8
quality_filter = df['total_votes_received'] > 2

test_df = df[mode_filter]
print(f'there are {len(test_df)} test samples')

# set up dataloader to predict all samples in test dataset
transform_test = transforms.Compose([extremes_remover(signal_max = 2000, signal_min = 20)])
combine_montage = CDAC_combine_montage()
con_combine_montage = con_combine_montage()
'''
test_dataset = BonoboDataset(test_df, config.PATH_FILES_BONOBO, 
                           transform=transform_val,
                           montage = combine_montage
                          )
test_dataloader = DataLoader(test_dataset, batch_size=32,shuffle=False,num_workers=os.cpu_count())
for x, y in test_dataloader:
    with torch.no_grad():
        print(x.shape)
        print(y)
        break

'''
#Bonobo_con = ContinousToSnippetDataset('/home/ubuntu/data/Bonobo01742_0.mat',montage=con_combine_montage)
#con_dataloader = DataLoader(Bonobo_con, batch_size=config.BATCH_SIZE,shuffle=False,num_workers=os.cpu_count())

# load pretrained model
model = FineTuning.load_from_checkpoint('/data/0shared/lijun/code/eeg/Spike_37chan/Models/YOUR_MODEL_NAME/1s-round11-hardmine-chan_weights-v1.ckpt',
                                        lr=config.LR,
                                        head_dropout=config.HEAD_DROPOUT,
                                        n_channels=config.N_CHANNELS,
                                        n_fft=config.N_FFT,
                                        hop_length=config.HOP_LENGTH,
                                       )

#model = SCDNN.load_from_checkpoint('/data/0shared/lijun/code/eeg/Spike_37chan/Models/YOUR_MODEL_NAME/round9-SCDNN-hardmine-chan_weights.ckpt',
#    lr=config.LR,
#    n_channels=config.N_CHANNELS,
#)# init trainer

trainer = pl.Trainer(fast_dev_run=False,enable_progress_bar=False,devices=[7],accelerator="gpu",strategy ='ddp')

# store results
#path_controls = os.path.join('/run/media/exx/Expansion/code/Spike_37chan/Judge/testset_controls.csv')
path_controls = os.path.join("/data/0shared/lijun/code/eeg/Spike_37chan/controlset.csv")
#path_controls = os.path.join("/data/0shared/lijun/code/eeg/EEG_level/eeg_level.csv")


controls = pd.read_csv(path_controls)
i = 0
#controls = controls[controls['Mode']=='Test']
for eeg_file in tqdm(controls.EEG_index):
    path = '/data/0shared/eeg/bonobo/continuousEEG/'+eeg_file+'.mat'
    Bonobo_con = ContinousToSnippetDataset(path,montage=con_combine_montage,transform=transform_test,window_size=1)
    con_dataloader = DataLoader(Bonobo_con, batch_size=128,shuffle=False,num_workers=os.cpu_count())
    
    preds = trainer.predict(model,con_dataloader)
    #preds = [np.squeeze(p) for p in preds]  # Ensure each part is 1D

    preds = np.concatenate(preds)
    preds = preds.astype(float)

    preds = pd.DataFrame(preds)
    #preds.to_csv(path_model+'/con_hardmine/'+ eeg_file +'.csv',index=False)
    preds.to_csv(path_model+'/con_hardmine_12/'+ eeg_file +'.csv',index=False)

    