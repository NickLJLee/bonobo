...
# load dataset
df = pd.read_csv('/run/media/exx/Expansion2/code/Spike_37chan/hard_mining.csv',sep=',') # ; -> ,
# add transformations
...
callbacks = [ModelCheckpoint(dirpath=model_path, filename='hardmine-chan_weights', monitor='val_loss')]
# create trainer, use fast dev run to test the code
trainer = pl.Trainer(devices=1, accelerator="gpu", min_epochs=5,max_epochs=150,logger=wandb_logger,callbacks=callbacks,fast_dev_run=False)
# train the model
